...
<table width="100%" border="0"  cellpadding=3 cellspacing=1 bgcolor=#999999 align="center">
  <colgroup>
	<col width="63" /> <?php //1. 번호?>
	<col width="80" /> <?php //2. 업로드일자?>
	<col width="100" /><?php //3. 받는분성명?>
	<col width="110" /><?php //4. 받는분전화번호?>
	<col width="139" /><?php //5. 주문자성명?>
	<col width="110" /><?php //6. 주문자전화번호?>
	<col width="189" /><?php //7. 주문자이메일?>
	<col width="80" /> <?php //8. 기념일?>
	<col width="190" /><?php //9. 기념일 내용?>
	<col width="80" /> <?php //10.동영상?>
	<col width="80" /> <?php //11.공개여부?>
	<col width="80" /> <?php //12.접근경로?>
	<col width="80" /> <?php //13.등록시간?>
	<col width="80" /> <?php //14.엑셀 등록일?>
  </colgroup>
     	...
     	<table cellpadding="0" cellspacing="0" width="100%" border="0" style="table-layout:fixed">
          ...
          <?
		    ...
		    if($skey !=""){
		  	...
		  	$sql="\n";
		  	$sql=$sql."SELECT UPDAY,RNAME,RTEL,SNAME,STEL,SEMAIL,EVNTDAY,EVNTCONT,URL,ISPUBLIC,ACESS,UPDAYTIME,WDATE \n";
		  	$sql=$sql."FROM TBL_EVNT_UPLOAD_DB \n";
		  	$sql=$sql."WHERE INTNUM IS NOT NULL ".$sub_query." \n";
		  	...
		  	while($row=mysql_fetch_array($rs)){
		  		...
		  		$sEmail = $row['SEMAIL'];
		  		$evntDay = $row['EVNTDAY'];
		  		$evntCont = $row['EVNTCONT'];
		  		$url = $row['URL'];
		  		$isPublic = $row['ISPUBLIC'];
		  		$acess = $row['ACESS'];
		  		$updayTime = $row['UPDAYTIME'];
		  		$wdate = $row['WDATE'];
		  ...
		  <tr align="center" bgcolor="<?=($intLoop%2)?"#f2f5f9":"#ffffff"?>" height="25">
		     ...
		     <td><?=$sEmail?></td>
		     <td><?=$evntDay?></td>
		     <td align="left" style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap;" title="<?=$evntCont?>"><?=$evntCont?></td>
		     <td>
		     	<?
